...
namespace StockMaster.Data
{
    ...
    {
        /*
            Сама головна і бісячі проблема тут це трекінг обєктів.
            ...
            Основна проблема не в обновлені данних без мого типу "відома", а в тому що EF
                в один момент часу може слідкувати тільки за один об'єктом з моделей
                ...
                щоб був Update бо без нього воно просто створе новий об'єкт.
                Загалом це ще пів біди. Ще проблема в тому що ми насправді може вручну прописати щоб EF неслідив за об'єктом,
                але це працює тільки з витягуванням данних. Типу просто прописав _context.MODEL.AsNoTracking().QUERY але проблема
                в тому що це працює тільки з ВИТЯГУВАННЯМ даних. Тобто я маю наувазі що EF починає слідкувати за об'єктом після того 
                як ти його витянув і навіть після того як ти його обновив чи видали, але для видалить і обновить немає можливості прописати 
                AsNoTracking() тому треба вручну очищати слідкування об'єкта в моменті.
         */
